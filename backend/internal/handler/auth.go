...
func (h *AuthHandler) SendMagicLink(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil || body.Email == "" {
		jsonError(w, ErrEmailRequired, http.StatusBadRequest)
		return
	}
	...
	if !validEmail(email) {
		...
		jsonError(w, ErrInvalidEmail, http.StatusBadRequest)
		return
	}
	...
	token, err := h.auth.SendMagicLink(r.Context(), email, body.Locale)
	if err != nil {
		...
		jsonError(w, ErrSendLinkFailed, http.StatusInternalServerError)
		return
	}
...
func (h *AuthHandler) Verify(w http.ResponseWriter, r *http.Request) {
	...
	if token == "" {
		jsonError(w, ErrTokenRequired, http.StatusBadRequest)
		return
	}
	user, jwt, err := h.auth.VerifyToken(r.Context(), token)
	if err != nil {
		jsonError(w, ErrInvalidToken, http.StatusUnauthorized)
		return
	}
...
func (h *AuthHandler) Me(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		jsonError(w, ErrUserNotFound, http.StatusNotFound)
		return
	}
	jsonOK(w, user)
}
